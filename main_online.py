...
class Workspace:
    ...
    def run(self):
        ...
        self.agent.eval()  # Ensure the agent is in evaluation mode if necessary
        ...
        while True:
            # Action selection logic based on the state of training
            if self.global_frames < self.cfg.start_steps:
                action = train_env.action_space.sample()  # Random action if before start steps
            else:
                action, _ = self.agent.get_action(obs, training=True)  # Agent selects action

            # Execute the action in the environment and observe the next state and reward
            next_obs, reward, done, info = train_env.step(action)

            # Collect data from the step and store in the current trajectory
            ...
            if done:
                # Episode has ended, check if the trajectory can be added to storage
                self.agent.add_trajectory(current_trajectory)
                if not self.agent.add_trajectory(current_trajectory):
                    print("Warning: Unable to add trajectory - storage may be full.")
                    break  # Exit loop if storage is full

                # Reset the current trajectory for the next episode
                current_trajectory = {'observations': [], 'actions': [], 'rewards': [], 'dones': []}
                obs = train_env.reset()  # Reset environment for a new episode

                if self.global_frames >= self.cfg.num_steps:
                    break  # Exit loop if the maximum number of frames has been reached

        # After exiting the loop, you may need to handle any cleanup or finalization tasks.
...
from main_online import Workspace as W # type: ignore
from main_online import Config as C # type: ignore
